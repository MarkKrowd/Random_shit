...
for lat_deg in range(-90,90,10):
    ...
    for values in list_lon:
        Est = values/180*np.pi
        E.append(Est)
    for values in list_lat:
        Nord = np.log(np.tan(np.pi/4+values/180*np.pi/2))
        N.append(Nord)
...
for lists in List_X:
    if index > 1:
        if len(lists)>1:
            ...
            E = []
            N = []
            for values in List_lon[index]:
                Est = values/180*np.pi
                E.append(Est)
            for values in List_lat[index]:
                Nord = np.log(np.tan(np.pi/4+values/180*np.pi/2))
                N.append(Nord)
            #print(x,y,z)
...
for lists in lambs:
    E = []
    N = []
    for values in lambs[index]:
        Est = values/180*np.pi
        E.append(Est)
    for values in phis[index]:
        Nord = np.log(np.tan(np.pi/4+values/180*np.pi/2))
        N.append(Nord)
