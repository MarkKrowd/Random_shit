...
for lat_deg in range(-90,100,10):
    list_lon = []
    list_lat = []
    ...
    plt.plot(list_lon,list_lat,linewidth=0.5,color='gray')  
...
for lists in List_X:
    if index > 1:
        if len(lists)>1:
            ...
            plt.fill(List_lon[index],List_lat[index],c='gray')
...
for lists in lambs:
    ...
    plt.plot(lambs[index],phis[index],linewidth = 1,color='blue')
