...
def arc_G_cercle(depart,arrivee):
    x_dep,y_dep,z_dep = sph2cart(depart[0],depart[1],1)
    x_arr,y_arr,z_arr = sph2cart(arrivee[0],arrivee[1],1)
...
for i in range(len(Lambs)):
    a,b,c = sph2cart(Lambs[i],phiis[i],1)
    X.append(a)
    Y.append(b)
    Z.append(c)
    
ax.plot3D(X,Y,Z,linewidth = 1,color='green')
ax.plot3D([-0.001,0.001],[-0.001,0.001],[-0.001,0.001],linewidth = 4,color='yellow')
