...
for lat_deg in range(-90,90,10):
    ...
    for i in range(len(list_lon)):
        phi_zero = np.pi/3
        lamb = list_lon[i]/180*np.pi
        phii = list_lat[i]/180*np.pi
    ...
    if index > 1:
        if len(lists)>1:
            ...
            for i in range(len(List_lon[index])):
                phi_zero = np.pi/3
                lamb = List_lon[index][i]/180*np.pi
                phii = List_lat[index][i]/180*np.pi
                alpha = np.cos(phii)/(phi_zero-phii+1/np.tan(phi_zero))*lamb
                rho = 6376000*(phi_zero-phii+1/np.tan(phi_zero))
                Est = rho*np.sin(alpha)
                E.append(Est)
...
for lists in lambs:
    ...
    for i in range(len(lambs[index])):
        phi_zero = np.pi/3
        lamb = lambs[index][i]/180*np.pi
        phii = phis[index][i]/180*np.pi
        alpha = np.cos(phii)/(phi_zero-phii+1/np.tan(phi_zero))*lamb
        rho = 6376000*(phi_zero-phii+1/np.tan(phi_zero))
        Est = rho*np.sin(alpha)
        E.append(Est)
